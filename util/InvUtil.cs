...
namespace Invoice365
{
    class InvUtil
    {
        ...
        {
            DbDataReader rdr = DB.getInstance(DB.MSSQL).ExecuteQuery("select company,street1,street2,city,state,zip,phone,s_street1,s_street2,s_city,s_state,s_zip,s_phone from customers where company='" + customer + "'");
            ...
            rdr = DB.getInstance(DB.MSSQL).ExecuteQuery("select company,street1,street2,city,state,zip,phone from billers where company='" + biller + "'");
